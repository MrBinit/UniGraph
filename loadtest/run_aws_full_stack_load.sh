...
LOCAL_REDIS_PORT="$(yaml_get runtime.local_redis_port)"
...
echo "[aws-load] starting local redis on localhost:${LOCAL_REDIS_PORT}..."
LOCAL_REDIS_PORT="${LOCAL_REDIS_PORT}" docker compose -f docker-compose.yml --profile local-redis up -d redis >/dev/null
...
export REDIS_APP_PORT="${REDIS_APP_PORT:-${LOCAL_REDIS_PORT}}"
export REDIS_WORKER_HOST="${REDIS_WORKER_HOST:-127.0.0.1}"
export REDIS_WORKER_PORT="${REDIS_WORKER_PORT:-${LOCAL_REDIS_PORT}}"
